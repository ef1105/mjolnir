...
Air Combat
- hle for dsps incomplete
- needs i/o review
	
Cybersled
- needs i/o review
- dsp shared ram not managed properly - flickering floor
...
extern void namcos2_set_posirq_scanline( int scanline );

static UINT16 winrun_posirq_scanline;
static WRITE16_HANDLER( winrun_gpu_posirq_w)
{
	COMBINE_DATA( &winrun_posirq_scanline );
	namcos2_set_posirq_scanline( 0x50+0x89 ); /* hack; posirq is accumlative */
}
static READ16_HANDLER( winrun_gpu_posirq_r)
{
	return winrun_posirq_scanline;
}
...
void
namcos21_EnableDSPs( void )
{
	if( namcos2_gametype != NAMCOS21_WINRUN91 )
	{
		if( namcos21_dspram16[0] == 0x524f )
		{
			UINT16 *pMem = (UINT16 *)memory_region(CPU_DSP_MASTER_REGION);
			int i;
			for( i=0; i<2; i++ )
...
InitDSP( void )
{
	 ...
	 *
	 * Note: Winning Run contains: "ACO PRESENTS. PRGRAMMED BY TAM."
	 */
...
{ /* same address space as namcos21_pointram? */
	pointrom_idx<<=16;
	pointrom_idx|=data;
} /* dsp_port3_w */
...
{
//	printf("xf(%d) 0x%4x\n",data,mpDspState->slaveOutputSize);
	logerror("xf(%d)\n",data);
}
...

	{
		...
		if( count && mpDspState->slaveOutputSize > count )
		{
			...
			else
			{
				...
				for(;;)
				{
					UINT8 code = namcos21_pointram[quad_idx++];
					color = namcos21_pointram[quad_idx++]|(code<<8);
					for( j=0; j<4; j++ )
					{
						UINT8 vi = namcos21_pointram[quad_idx++];
...
{
	return namcos21_pointram[pointram_idx];
} /* pointram_data_r */
...
//		logerror( " %02x", data );
		namcos21_pointram[pointram_idx++] = data;
...
static WRITE16_HANDLER( cybsled_unk_w )
{ // ???
//	printf( "%02x:%04x\n", offset*2, data );
}
...
static ADDRESS_MAP_START( namcos21_68k_slave, ADDRESS_SPACE_PROGRAM, 16 )
	AM_RANGE(0x040000, 0x04003f) AM_WRITE(cybsled_unk_w)
...
winrun_flushpoly( void )
{
	if( winrun_poly_index>0 )
	{
		...
		else
		{
			...
			{
				UINT8 code = namcos21_pointram[quad_idx++];
				color = namcos21_pointram[quad_idx++];
				for( j=0; j<4; j++ )
				{
					UINT8 vi = namcos21_pointram[quad_idx++];
					sx[j] = NAMCOS21_POLY_FRAME_WIDTH/2  + (INT16)pSource[vi*3+0];
...
static WRITE16_HANDLER( winrun_dsp_irq_enable_w )
{
	winrun_irq_enable = data;
	if( data )
	{
		winrun_flushpoly();
...
static ADDRESS_MAP_START( winrun_dsp_io, ADDRESS_SPACE_IO, 16 )
	...
	AM_RANGE(0x0c,0x0c) AM_WRITE(winrun_dsp_irq_enable_w)
...
static ADDRESS_MAP_START( am_master_winrun, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x700000, 0x700001) AM_WRITE(MWA16_NOP) /* 0x012c */
...
static ADDRESS_MAP_START( am_gpu_winrun, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0xe0000c, 0xe0000d) AM_READ(winrun_gpu_posirq_r) AM_WRITE(winrun_gpu_posirq_w)
ADDRESS_MAP_END
...
static MACHINE_DRIVER_START( s21base )
	...
	MDRV_CPU_ADD(TMS32025,24000000*8) /* 24 MHz?; overclocked */
...
static MACHINE_DRIVER_START( winrun_c140_typeB )
	...
	MDRV_CPU_ADD(TMS32025,24000000*5) /* 24 MHz? overclocked */
	MDRV_CPU_PROGRAM_MAP(winrun_dsp_program,0)
	MDRV_CPU_DATA_MAP(winrun_dsp_data,0)
	MDRV_CPU_IO_MAP(winrun_dsp_io,0)
...
//	ROM_LOAD( "sys2c65b.bin",  0x008000, 0x008000, CRC(e9f2922a) SHA1(5767d2f85e1eb3de19192e73b02221f28b1fbb83) )
	...
	ROM_LOAD( "de1_obj4.4s",  0x040000, 0x40000, CRC(335f0ea4) SHA1(9ec065d99ad0874b262b372334179a7e7612558e) )
	ROM_LOAD( "de1_obj1.5x",  0x080000, 0x40000, CRC(45f2334e) SHA1(95f277a4e43d6662ae44d6b69a57f65c72978319) )
	ROM_LOAD( "de1_obj5.4x",  0x0c0000, 0x40000, CRC(9e22999c) SHA1(02624186c359b5e2c96cd3f0e2cb1598ea36dff7) )
	ROM_LOAD( "de1_obj2.3s",  0x100000, 0x40000, CRC(8f1a542c) SHA1(2cb59713607d8929815a9b28bf2a384b6a6c9db8) )
	ROM_LOAD( "de1_obj6.2s",  0x140000, 0x40000, CRC(346df4d5) SHA1(edbadb9db93b7f5a3b064c7f6acb77001cdacce2) )
	ROM_LOAD( "de1_obj3.3x",  0x180000, 0x40000, CRC(fc94544c) SHA1(6297445c64784ee253716f6438d98e5fcd4e7520) )
	...
	ROM_LOAD16_BYTE( "de1_du.3a",  0x00000, 0x80000, CRC(fe65d2ab) SHA1(dbe962dda7efa60357fa3a684a265aaad49df5b5) )
	ROM_LOAD16_BYTE( "de1_dl.1a",  0x00001, 0x80000, CRC(9bb37aca) SHA1(7f5dffc95cadcf12f53ff7944920afc25ed3cf68) )
...
ROM_START( winrun )
	...
	ROM_LOAD16_BYTE( "sg1d0u.3a",  0x00000, 0x20000, CRC(1dde2ac2) SHA1(2d20a434561c04e48b52a2137a8c9047e17c1013) )
	ROM_LOAD16_BYTE( "sg1d0l.1a",  0x00001, 0x20000, CRC(2afeb77e) SHA1(ac1552f6e2788158d3477b6a0981d001d6cbdf13) )
	ROM_LOAD16_BYTE( "sg1d1u.3b",  0x40000, 0x20000, CRC(5664b09e) SHA1(10c1c29614eee2cffcfd69085f0450d81ba2e25f) )
	ROM_LOAD16_BYTE( "sg1d1l.1b",  0x40001, 0x20000, CRC(2dbc7de4) SHA1(824304c95942c7296f8e8dcf8ee7e22bf56154b1) )

	ROM_REGION16_BE( 0x80000, REGION_USER2, 0 ) /* 3d objects */
	ROM_LOAD16_BYTE( "sg1pt0u.8j", 0x00000, 0x20000, CRC(160c3634) SHA1(485d20d6cc459f17d77682201dee07bdf76bf343) )
	ROM_LOAD16_BYTE( "sg1pt0l.8d", 0x00001, 0x20000, CRC(b5a665bf) SHA1(5af6ec492f31395c0492e14590b025b120067b8d) )
	ROM_LOAD16_BYTE( "sg1pt1u.8l", 0x40000, 0x20000, CRC(b63d3006) SHA1(78e78619766b0fd91b1e830cfb066495d6773981) )
	ROM_LOAD16_BYTE( "sg1pt1l.8e", 0x40001, 0x20000, CRC(6385e325) SHA1(d50bceb2e9c0d0a38d7b0f918f99c482649e260d) )

	ROM_REGION16_BE( 0x100000, REGION_USER3, 0 ) /* bitmapped graphics */
	ROM_LOAD16_BYTE( "sg1gd0u.1p",  0x00000, 0x40000, CRC(7838fcde) SHA1(45e31269eed1999b73c41c2f5d2c5bfbbdaf23df) )
	ROM_LOAD16_BYTE( "sg1gd0l.3p",  0x00001, 0x40000, CRC(4bd02b9a) SHA1(b2fdfd1c1325864aaad87f5358ab9bbdd79ff6ae) )
	ROM_LOAD16_BYTE( "sg1gd1u.1s",  0x80000, 0x40000, CRC(271db29b) SHA1(8b35fcf273b9aec28d4c606c41c0626dded697e1) )
	ROM_LOAD16_BYTE( "sg1gd1l.3s",  0x80001, 0x40000, CRC(a6c4da96) SHA1(377dbf21a1bede01de16708c96c112abab4417ce) )
...
static void namcos21_init( int game_type )
{
	...
	namcos21_pointram = auto_malloc(PTRAM_SIZE);
...
GAMEX( 1991, winrun91, 0,       winrun_c140_typeB, winrun,       winrun,   ROT0,    "Namco", "Winning Run 91", 	  GAME_NOT_WORKING ) /* working */
GAMEX( 1989, winrun,   0,       winrun_c140_typeB, winrun,       winrun,   ROT0,    "Namco", "Winning Run Suzuka Grand Prix", GAME_NOT_WORKING ) /* working */
/* 1988, Winning Run */
GAMEX( 1987, driveyes, 0,       driveyes,          winrun,       driveyes, ROT0,    "Namco", "Driver's Eyes",      GAME_NOT_WORKING|GAME_NO_SOUND )
