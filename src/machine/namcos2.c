...
extern void namcos21_EnableDSPs( void );
extern void namcos21_HaltDSPs( void );
...
ReadWriteC148( int cpu, offs_t offset, UINT16 data, int bWrite )
{
	...
	switch(addr)
	{
	case 0x1c0000: /* NAMCOS2_C148_0 level */
	case 0x1c2000: /* NAMCOS2_C148_1 level */
	case 0x1c4000: /* NAMCOS2_C148_2 level */
	case 0x1c6000: /* NAMCOS2_C148_CPUIRQ level */
	case 0x1c8000: /* NAMCOS2_C148_EXIRQ level */
	case 0x1ca000: /* NAMCOS2_C148_POSIRQ level */
	case 0x1cc000: /* NAMCOS2_C148_SERIRQ level */
	case 0x1ce000: /* NAMCOS2_C148_VBLANKIRQ level */
		if( !bWrite )
		{
		//	printf( "%d:0x%x unexpected read from C148[0x%x]\n", cpu_getactivecpu(), activecpu_get_pc(), addr );
		}
		break;
	...
	case 0x1d0000:
		if( bWrite )
		{
		//	printf( "%d:0x%x write 0x%x to C148[0x%x]\n", cpu_getactivecpu(), activecpu_get_pc(), data, addr );
	...
	case 0x1d2000:
		if( bWrite )
		{
		//	printf( "%d:0x%x unexpected write 0x%x to C148[0x%x]\n", cpu_getactivecpu(), activecpu_get_pc(), data, addr );
		}
...
static int mNextScanline;

void namcos2_set_posirq_scanline( int scanline )
{
	/*
	if( scanline>mNextScanline && scanline<480 )
	{
		if( namcos2_68k_gpu_C148[NAMCOS2_C148_POSIRQ] )
		{
			timer_set( cpu_getscanlinetime(scanline), scanline, namcos2_68k_gpu_posirq );
		}
	}*/
	mNextScanline = scanline;
}

void namcos2_68k_gpu_posirq( int scanline )
{
	printf( "POSIRQ!\n" );
	force_partial_update(scanline);
...
{
	if( mNextScanline>=0 && mNextScanline<480 )
	{
		if( namcos2_68k_gpu_C148[NAMCOS2_C148_POSIRQ] )
		{
			timer_set( cpu_getscanlinetime(mNextScanline), mNextScanline, namcos2_68k_gpu_posirq );
		}
	}
	if( namcos2_68k_gpu_C148[NAMCOS2_C148_VBLANKIRQ] )
	{
