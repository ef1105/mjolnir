...
//	sprintf(title, APPNAME ": %s [%s]", Machine->gamedrv->description, Machine->gamedrv->name);
	sprintf(title, "Mjolnir: %s [%s]", Machine->gamedrv->description, Machine->gamedrv->name);
	...
	win_video_window = CreateWindowEx( win_window_mode ? WINDOW_STYLE_EX : FULLSCREEN_STYLE_EX,
			...
			20, 20, 640, 480, NULL, menu, GetModuleHandle(NULL), NULL);
	if (!win_video_window)
		return 1;
...
int win_create_window(int width, int height, int depth, int attributes, double aspect)
{
   ...
   if( gUseOpenGL )
   {
      return poly3d_Init( win_video_window, width, height );
   }
   ...
   if( gUseOpenGL )
	{
		...
		poly3d_ResizeWindow( fill.right - fill.left, fill.bottom - fill.top );
		return;
	}
...
void win_constrain_to_aspect_ratio(RECT *rect, int adjustment, int constraints, int coordinate_system)
{
	...
	if( gUseOpenGL )
	{
//		poly3d_ResizeWindow( rect->right-rect->left, rect->bottom-rect->top );
	}
}
...
void win_adjust_window(void)
{
	...
	else
	{
		...
		if( gUseOpenGL )
		{
		//	poly3d_ResizeWindow(window.right-window.left,window.bottom-window.top);
		}
	}
